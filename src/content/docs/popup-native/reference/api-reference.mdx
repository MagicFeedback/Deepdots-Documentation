...
- `setMetric(key: String, value: Any?)`: measurable values, sent in the
  dedicated `feedback.metrics` field of the payload. Re-sent on every flush
  and held in memory only, like the attributes above.
