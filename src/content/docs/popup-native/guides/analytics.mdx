...
- **Blank keys and `null` values are ignored**: the call does nothing.
- **In memory only**: "persistent" means re-sent in every batch while the instance lives, not stored. Restarting the app clears the metrics, and `setUserId()` discards them along with the previous user's data.
