...
:::caution[No sobreviven a reiniciar la app]
Los atributos viven en memoria, en la instancia del SDK: matar o reiniciar la app los borra. Vuelve a fijarlos después de cada `init()`. `setContactAttributes`, más abajo, es la contraparte persistente.
:::

Siguen las mismas reglas que las [métricas](#métricas): los valores se convierten a texto, las claves en blanco y los valores `null` se ignoran, y la llamada no hace nada mientras el tracking está desactivado. Un lote solo sale si hay eventos pendientes, así que unos atributos fijados sin actividad posterior viajan en el primer lote con contenido.
...
- **Las claves en blanco y los valores `null` se ignoran** — la llamada no hace nada.
- **Solo en memoria** — «persistente» significa que se reenvía en cada lote mientras viva la instancia, no que se almacene. Reiniciar la app borra las métricas, y `setUserId()` las descarta junto con los datos del usuario anterior.
